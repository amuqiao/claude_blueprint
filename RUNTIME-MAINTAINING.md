...
- 允许按需维护领域规则目录，例如 `rules/backend/`；领域规则不默认等同于全局规则，应由新项目或具体任务按需引用
...
- `configuration-design.md`：配置项设计、派生与联动校验规则
- `file-naming.md`：文件命名、后缀与重命名规则
- `backend/`：后端工程领域规则，按新服务、FastAPI、部署或入口任务按需加载，不作为所有任务默认上下文
...
- 如果一条规则回答的是“配置项应该如何暴露、派生和校验”，放 `configuration-design.md`
- 如果一条规则回答的是“文件该怎么命名或重命名”，放 `file-naming.md`
- 如果一条规则回答的是“后端服务如何分层、部署、配置、鉴权、记录日志或实现任务契约”，放 `backend/` 的对应子目录
...
- 不要把方法论、平台教程或具体项目排障案例直接塞进 `rules/backend/`
