...
SOURCE_ROLE_INDEX="$SCRIPT_DIR/ROLE_INDEX.md"
...
if [ ! -f "$SOURCE_ROLE_INDEX" ]; then
  err "未找到全量角色索引: $SOURCE_ROLE_INDEX"
  exit 1
fi
...
echo ""
echo "Web UI 中优先选择「专家角色全量模式」复现固定角色子 agent；只验证通用 subagent/workflow 时可用「标准模式」或「PTC 模式」。"
