...
ROLE_ALLOWLIST="$SCRIPT_DIR/ROLE_ALLOWLIST.txt"
PYTHON_BIN="${PYTHON_BIN:-python3}"

DRY_RUN=false
FORCE=false
VERIFY=false
PRESET_ID="codex-roles-lite"
...
选项:
  ...
  --preset-id=ID               Harness preset id；默认 codex-roles-lite
...
echo -e "${BOLD}"
echo "╔════════════════════════════════════════════════════════════╗"
echo "║   DeepSeek Harness Lite Expert Role Preset 安装           ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
if [ -f "$ROLE_ALLOWLIST" ]; then
  ...
  ok "lite 角色清单存在: $EXPECTED_COUNT 个"
else
  err "未找到 lite 角色清单: $ROLE_ALLOWLIST"
  exit 1
fi
...
step "2/4" "生成 Harness lite preset"
ARGS=(
  "$CONVERTER"
  "--source-dir=$SOURCE_DIR"
  "--dsh-home=$DSH_HOME_VALUE"
  "--preset-id=$PRESET_ID"
  "--include-roles-file=$ROLE_ALLOWLIST"
)
...
if $DRY_RUN; then
  ...
  if [ "$COUNT" != "$EXPECTED_COUNT" ]; then
    err "生成角色文件数量不匹配: allowlist=$EXPECTED_COUNT generated=$COUNT"
    exit 1
  fi
  if [ "$RUNTIME_ROLE_COUNT" != "$EXPECTED_COUNT" ]; then
    err "运行时入口角色数量不匹配: allowlist=$EXPECTED_COUNT runtime=$RUNTIME_ROLE_COUNT"
    exit 1
  fi
...
if $VERIFY && ! $DRY_RUN; then
  ...
  if command -v dsh >/dev/null 2>&1; then
    ok "dsh $(dsh --version)"
    echo "请重启 dsh web，然后在 Web UI 新建会话时选择「专家角色精简模式」。"
  else
