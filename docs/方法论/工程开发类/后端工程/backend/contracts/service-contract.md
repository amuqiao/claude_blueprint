...
服务必须统一时间字段语义，避免同一类字段在不同接口中混用 Unix 秒、Unix 毫秒和本地时间字符串。

时间字段规则：

| 字段类型 | 格式 | 规则 |
| --- | --- | --- |
| `server_time` | Unix 秒级整数 | 只用于 envelope 中的服务端当前时间，便于调用方对齐时钟。 |
| 对外业务时间 | ISO 8601 UTC 字符串，例如 `2026-06-20T10:00:00Z` | 用于 `created_at`、`started_at`、`finished_at`、`next_retry_at`、资源过期时间等业务字段。 |
| 日志 `timestamp` | ISO 8601 UTC 字符串 | 用于结构化日志事件时间。 |
| 数据库存储时间 | timezone-aware UTC 时间 | 数据库层不得保存无时区本地时间；展示时再投影为对外格式。 |
| 耗时和超时 | 带单位字段名 | 使用 `duration_ms`、`timeout_seconds`、`expires_in_seconds` 等明确单位，禁止裸 `timeout`、`duration`。 |

公开契约中不得让同一个字段有时返回秒、有时返回毫秒，或有时返回 ISO 字符串。确需同时提供机器排序和人读时间时，应使用不同字段名，并在接口文档中说明各自用途。
...
新增接口的字段表、必选/可选、类型、约束、`null` 与省略语义、示例、错误码和验收清单，按 [`api-operation-template.md`](api-operation-template.md) 编写。envelope、错误、JobView、CallbackEnvelope 与业务 schema 的组合复用，按 [`schema-composition.md`](schema-composition.md) 落地。
...
新增 HTTP 接口的字段表、示例、错误码表和完整接入验收以 [`api-operation-template.md`](api-operation-template.md) 为准。本文只保留服务级基线要求：

- 成功响应套统一 envelope。
- 错误响应使用服务级错误码和项目子错误码。
- HTTP status 与错误语义一致，不全部返回 `200`。
- OpenAPI 或接口文档是 schema 和 contract tests 的投影，不成为第二套事实源。
