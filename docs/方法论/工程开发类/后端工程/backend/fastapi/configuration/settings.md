...
根 Settings 只负责聚合子配置、读取统一配置源和校验跨子对象不变量。子对象负责本领域字段的类型、默认值、敏感字段保护和领域内校验。

子对象必须满足：

- 每个子对象有清晰职责，不承载其他领域配置。
- 数据库配置独立成 `DatabaseSettings` 或等价对象；数据库 URL、连接池、迁移和只读/读写边界不得混入 AI、Job 或 HTTP 配置。
- Job 的并发、积压、超时主控、恢复扫描和 callback 领取窗口归入 `JobSettings` 或等价对象。
- AI 供应商的 base URL、模型路由、主 timeout、代理、组织或租户标识归入 `AIProviderSettings` 或等价对象；密钥仍按敏感字段处理。
- 子对象之间不得互相读取环境变量或重新构造 Settings；跨对象派生关系只能在根 Settings 的统一校验阶段完成。
- 子对象可以使用嵌套模型、前缀 env key 或显式 alias，但 `.env.example` 仍是应用配置键集合的单向真源。

不要为了拆分文件而制造第二套配置事实源。`.env.example`、字段到 env key 的映射清单、废弃键拒绝清单和机器检查仍按同一套应用配置语义维护。
