...
| `complex-workflow.md` | 复杂 Job 内部执行计划、work item、merge、finalize 和 Celery Canvas / Taskiq 编排映射边界 | 公开 Job 状态机、执行器教程、业务 handler 实现 |
...
5. `complex-workflow.md`：当单个 Job 内部需要分片、并行、merge、finalize 或执行器编排时读取。
6. `executors/celery.md` 或 `executors/taskiq.md`：按项目选型读取一个执行器规则。
7. `../fastapi/jobs/async-job.md`：当 API 框架是 FastAPI 时读取。
8. `../typer/ops-cli.md`：当运行时排障或运维命令使用 Typer 时读取。
