    ...
    routes: [
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return EditorScreen(diaryId: id);  // id 为 null 表示新建
        },
      ),
      GoRoute(
...
abstract class Routes {
  ...
  static const editorWithId = '/editor/:id';
...
```
go       替换整个导航栈，用于底部导航栏切换 tab、登录后跳首页
push     叠加到当前栈，用于进入详情页、打开编辑器
replace  替换当前页，用于登录页跳首页后不想让用户回退到登录页
```
...
错误的典型案例：用 `go` 打开编辑器，保存后用户按返回按钮直接回到了根路由，丢失了打开编辑器前的页面状态。
...
```
路径参数（/editor/:id）   用于标识资源，是 URL 的一部分，会出现在导航历史里
查询参数（/editor?mode=new）  用于影响展示的可选参数，不是资源标识
```

具体约定：

```
日记 ID        -> 路径参数    /editor/diary-123
编辑器初始模式  -> 查询参数    /editor?mode=template
```
...
// 正确：在路由配置里转换
GoRoute(
  path: Routes.editorWithId,
  builder: (context, state) {
    final id = state.pathParameters['id'];
    // id 为 null 时 EditorScreen 视为新建模式
    return EditorScreen(diaryId: id);
  },
),
...
```dart
// 错误：通过路由 extra 传对象
context.push(Routes.editor, extra: diaryObject);

// 正确：只传 ID，Screen 里通过 Provider 加载
context.push('/editor/${diary.id}');
```
...
```dart
// 打开编辑器，等待结果
final result = await context.push<bool>(Routes.editor);
if (result == true) {
  // 用户保存了，刷新列表
...
如果页面内有 tab 切换（如首页内的不同视图），优先用 Widget 状态管理，不用路由嵌套。只有以下情况使用嵌套路由：
...
TimelineCard(
  diary: diary,
  onTap: () => context.push('/editor/${diary.id}'),
)
...
class TimelineCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => context.push('/editor/${diary.id}'),  // 叶子组件不应该知道路由
    );
  }
}
```
