...
```
Entity        {Name}Entity      ItemEntity, CategoryEntity
DTO           {Name}DTO         ItemDTO, UserProfileDTO
Domain Model  直接用领域名称    Item, Category, UserProfile
```
...
abstract class ItemRepository {
  Future<List<Item>> fetchByDateRange(DateTime start, DateTime end);
  Future<Item> fetchById(String id);
...
```
AsyncNotifier     {Name}ListNotifier   ItemListNotifier
Notifier          {Name}Notifier       CategoryFilterNotifier
...
class ItemListNotifier extends _$ItemListNotifier {
  ...
  Future<List<Item>> _fetch() {
    ...
    return ref.read(itemRepositoryProvider).fetchByDateRange(
      filter.startDate,
      filter.endDate,
    );
  }
...
// 错误：通过构造函数注入
class ItemListNotifier extends _$ItemListNotifier {
  ItemListNotifier(this._filter);  // 不要这样做
...
```dart
// 正确：DAO 返回 Entity
Future<List<ItemEntity>> getByDateRange(DateTime start, DateTime end);

// 错误：DAO 返回 Domain Model（绕过了 Repository 的转换职责）
Future<List<Item>> getByDateRange(DateTime start, DateTime end);
```
...
MigrationStrategy get migration => MigrationStrategy(
  ...
  onUpgrade: (m, from, to) async {
    if (from < 2) {
      await m.addColumn(itemTable, itemTable.status);
    }
  },
);
```
...
├── data/
│   ├── models/
│   │   ├── domain/         Domain Model（Item, Category 等）
│   │   ├── entities/       数据库 Entity
│   │   └── dtos/           网络 DTO
│   ├── repositories/
