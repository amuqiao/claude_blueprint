...
如果一份文档回答不了这三个问题，它还不是组件架构文档，只是页面草图。
...
```
HomeScreen                        页面入口，组装所有 section，持有页面级导航逻辑
├── HeatMapSection                热力图区域，展示年度打卡分布，处理日期点选事件
│   └── HeatMapCell               单个日期格子，展示颜色深度，触发点选回调
├── TagFilterBar                  标签筛选栏，展示可选标签列表，持有当前选中状态
│   └── TagChip                   单个标签，展示选中/未选中状态，触发切换回调
└── HomeTimeline                  时间轴容器，按天分组展示日记列表，响应定位指令
    ├── DayGroup                  单天分组，展示日期标题和当天的记录列表
    │   └── TimelineCard          单条日记卡片，展示摘要信息，触发详情跳转
...
命名用 PascalCase，不要缩写。`HeatMapCell` 比 `HMCell` 好，三个月后你还认识前者。
...
```
状态                    Owner               读取方              写入触发
当前选中的标签列表       TagFilterBar        HomeTimeline        TagChip 点击
热力图点选的目标日期     HomeScreen          HomeTimeline        HeatMapCell 点击
时间轴滚动位置          HomeTimeline（内部）  HomeTimeline        系统 / 定位指令
日记列表数据            DiaryListProvider   HomeTimeline        Repository 返回
```
...
```
DiaryRepository
  └── fetchByDateRange(start, end) -> List<DiaryEntry>
        └── DiaryListNotifier（Riverpod AsyncNotifier）
              ├── 监听 selectedTags 变化，触发重新 fetch
              └── 暴露 AsyncValue<List<DiaryEntry>>
                    └── HomeTimeline
                          └── DayGroup（按日期分组后传入）
                                └── TimelineCard（单条记录）
```
...
```
AsyncValue.loading   -> HomeTimeline 展示 skeleton
AsyncValue.error     -> HomeTimeline 展示错误提示，提供重试入口
