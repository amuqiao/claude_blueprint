...
| 阶段 | 输入 | 输出 | 进入下一步的条件 |
|------|------|------|------------------|
| 阶段 0：技术选型 | MVP 边界、首个核心路径、基础能力需求 | Tech Decision 文档、已锁定的关键依赖 | 状态管理、数据层、导航、缓存、测试方案不再边做边换 |
| 阶段 1：Slice 设计文档 | 已确定要进入 MVP 的功能或设计原则 | Slice 文档、交互语义、状态矩阵、验收条件 | 验收条件能指导实现和测试 |
| 阶段 2：组件架构文档 | Slice 文档 | 组件树、职责边界、状态归属、数据流 | 写 Widget 的人不用再猜组件怎么拆、状态放哪里 |
| 阶段 3：数据层实现 | 组件架构里的数据需求 | 数据模型、Repository、Provider / Notifier、错误边界 | 核心数据读写能独立跑通，不依赖 UI 临时兜底 |
| 阶段 4：UI 层实现 | 已跑通的数据层、组件架构文档 | Screen / Section / 叶子组件、导航和错误展示 | 主路径能在真实运行环境中走通 |
