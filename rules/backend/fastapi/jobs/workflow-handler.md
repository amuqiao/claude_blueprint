---
description: FastAPI 异步 Job workflow handler、job_type 与运行时扩展规则
---

# Workflow Handler 规则

Workflow Handler 规则适用于一个 FastAPI 异步 Job 服务需要支持多个 `job_type`、多个执行模式或多个业务处理器的场景。它补充异步 Job 规则，不替代状态机、队列投递、恢复扫描或外部集成规则。
...
`POST /jobs` 顶层字段应保持少量稳定字段，例如 `client_request_id`、`job_type`、`job_params`、`callback`、`metadata`、`options`。具体业务入参由 `job_type` 对应的 `job_params` schema 定义。
...
handler 注册必须有统一入口，例如 `register_all_workflows()`。API 进程和 Worker 进程都必须在处理请求或任务前完成注册。
...
异步 Job 的快照原则由 [`async-job.md`](async-job.md) 负责。Workflow Handler 只负责为不同 `job_type` 生成会影响执行语义的 `runtime_fields`，并保证 API 进程和 Worker 进程读取同一份已注册 handler 契约。
