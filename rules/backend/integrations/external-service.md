...
外部服务集成规则负责约束后端服务如何调用第三方或上游业务系统。它不规定具体供应商 API，也不替代业务接口文档、框架日志规则或 Job 状态机规则。
...
外部集成规则只定义哪些外部目标字段会影响执行语义；异步 Job 的快照结构、校验和读取方式由 Job 规则负责。
...
外部错误应转换为稳定业务错误码。不要把底层库异常、供应商内部错误或网络栈细节直接暴露给 API 响应。
