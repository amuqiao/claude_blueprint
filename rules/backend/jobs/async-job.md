...
执行器专项规则只能说明如何把本规则映射到具体技术；不得重新定义状态机、状态权威、恢复语义或终态幂等。多 `job_type` handler 细节见 [`workflow-handler.md`](workflow-handler.md)，外部 client 和对外写入副作用见 `../integrations/external-service.md`，部署形态和迁移执行见 `../deployment/service-deployment.md`。
